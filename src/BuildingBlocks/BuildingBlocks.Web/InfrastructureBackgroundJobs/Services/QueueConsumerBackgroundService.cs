...
namespace BuildingBlocks.Web.InfrastructureBackgroundJobs.Services;
...
public abstract class QueueConsumerBackgroundService<T> : BackgroundService
{
    ...
    protected QueueConsumerBackgroundService(IQueueReader<T> queue)
    {
        _queue = queue;
    }
    ...
    /// <summary>
    ///     Called when <see cref="ProcessItemAsync" /> throws a non-cancellation exception; default implementation is a
    ///     no-op.
    /// </summary>
