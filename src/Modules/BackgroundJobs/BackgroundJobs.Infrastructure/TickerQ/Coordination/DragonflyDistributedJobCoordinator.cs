...
public sealed class DragonflyDistributedJobCoordinator : IDistributedJobCoordinator
{
    ...
    )
    {
        ...
        bool acquired = await database.StringSetAsync(
            lockKey, lockValue, TimeSpan.FromSeconds(LeaseSeconds), when: When.NotExists);

        if (!acquired)
        {
            _logger.LogDebug(
                "Skipped background job {JobName} because another instance currently owns the coordination lease.", jobName);
            return;
        }

        using CancellationTokenSource executionCts = CancellationTokenSource.CreateLinkedTokenSource(ct);
    ...
    private IDatabase? HandleUnavailable(string jobName, string message, Exception? innerException = null)
    {
        if (!_options.TickerQ.FailClosed)
        {
            _logger.LogWarning(innerException,
        ...
        _logger.LogWarning(innerException,
            "Fail-closed coordination stopped background job {JobName}: {Message}", jobName, message);
    ...
    private async Task RenewLeaseAsync(
        IDatabase database, string key, string value, string jobName, CancellationTokenSource executionCts)
    {
        ...
        {
            long renewed = (long)await database.ScriptEvaluateAsync(
                RenewLeaseScript, new { key, value, leaseSeconds = LeaseSeconds });

            if (renewed != 0) continue;

            _logger.LogWarning(
                "Lost DragonFly coordination lease for background job {JobName}; cancelling the in-flight execution.", jobName);
            executionCts.Cancel();
            throw new InvalidOperationException(
