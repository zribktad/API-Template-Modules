...
namespace BackgroundJobs.Infrastructure.Validation;
...
public sealed class BackgroundJobsOptionsValidator : IValidateOptions<BackgroundJobsOptions>
{
    ...
    private static void ValidateTickerQ(TickerQSchedulerOptions options, List<string> failures)
    {
        ...
        if (!string.Equals(options.CoordinationConnection,
            TickerQSchedulerOptions.DefaultCoordinationConnection, StringComparison.OrdinalIgnoreCase))
            failures.Add($"BackgroundJobs:TickerQ:CoordinationConnection must be '{TickerQSchedulerOptions.DefaultCoordinationConnection}'.");
    }
    ...
    private static void ValidateCleanup(CleanupJobOptions options, List<string> failures)
    {
        ...
        ValidateNonNegative("BackgroundJobs:Cleanup:ExpiredInvitationRetentionHours", options.ExpiredInvitationRetentionHours, failures);
        ValidateNonNegative("BackgroundJobs:Cleanup:SoftDeleteRetentionDays", options.SoftDeleteRetentionDays, failures);
        ValidateNonNegative("BackgroundJobs:Cleanup:OrphanedProductDataRetentionDays", options.OrphanedProductDataRetentionDays, failures);
    }
    ...
    private static void ValidateEmailRetry(EmailRetryJobOptions options, List<string> failures)
    {
        ...
        ValidatePositive("BackgroundJobs:EmailRetry:MaxRetryAttempts", options.MaxRetryAttempts, failures);
        ValidatePositive("BackgroundJobs:EmailRetry:ClaimLeaseMinutes", options.ClaimLeaseMinutes, failures);
        ValidateNonNegative("BackgroundJobs:EmailRetry:DeadLetterAfterHours", options.DeadLetterAfterHours, failures);
    }

    private static void ValidateCron(string path, string cron, List<string> failures)
    {
        if (string.IsNullOrWhiteSpace(cron)) { failures.Add($"{path} is required."); return; }
        try { CrontabSchedule.Parse(cron); }
        catch (CrontabException) { failures.Add($"{path} must be a valid 5-part CRON expression."); }
    }

    private static void ValidatePositive(string path, int value, List<string> failures)
    {
        if (value <= 0) failures.Add($"{path} must be greater than 0.");
    }

    private static void ValidateNonNegative(string path, int value, List<string> failures)
    {
        if (value < 0) failures.Add($"{path} must be greater than or equal to 0.");
    }
}
