...
public sealed class ProductSoftDeleteCascadeRule : ISoftDeleteCascadeRule
{
    ...
    {
        if (
            entity is not Product product
            || dbContext is not ProductCatalogDbContext productCatalogDbContext
        )
            return [];
        ...
        return await productCatalogDbContext
            ...
            .Where(link =>
                link.ProductId == product.Id && link.TenantId == product.TenantId && !link.IsDeleted
            )
