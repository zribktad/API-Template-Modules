...
## Workflow: Migrations That Contain Raw SQL

Some migrations don't change the EF model at all — they only add database-level constructs that EF can't express (raw indexes using unsupported annotations, extensions, stored procedures, etc.). The correct workflow is:

### 1. Always generate first, edit second

Never write a migration file from scratch. Always let EF Core generate it:
...
If the model has no EF-tracked changes, EF generates an **empty** migration with the correct `.Designer.cs` and updates the snapshot. That is the right starting point.

### 2. Never edit the `.Designer.cs`

The `.Designer.cs` contains the embedded model snapshot at that migration's point in time. It is generated by EF and must stay that way. Editing it by hand risks breaking migration history and `dotnet ef migrations script` output.

### 3. If `dotnet ef migrations add` fails — fix the root cause

A failure during generation usually means a library or annotation conflict. Common example in this project: `EFCore.ComplexIndexes.PostgreSQL` intercepts the migration differ and rejects annotations it doesn't know (e.g. `Npgsql:TsVectorConfig` from `IsTsVectorExpressionIndex`).

The fix is to remove the conflicting EF configuration and implement the construct as raw SQL instead. Do **not** try to work around the error by patching tool invocations.

Typical resolution:
1. Remove the unsupported EF configuration from the entity type configuration file.
2. Write an equivalent SQL file (see below).
3. Re-run `dotnet ef migrations add` — it should now succeed.

### 4. Add raw SQL via embedded resources

Once you have a generated (possibly empty) migration file, add raw SQL calls inside it:

```csharp
protected override void Up(MigrationBuilder migrationBuilder)
{
    migrationBuilder.Sql(ModuleSqlResource.Load("my_index_v1_up.sql"));
}

protected override void Down(MigrationBuilder migrationBuilder)
{
    migrationBuilder.Sql(ModuleSqlResource.Load("my_index_v1_down.sql"));
}
```

SQL file naming convention: `{name}_v{version}_{up|down}.sql`. Always provide a `_down.sql` — even if it is a no-op comment, so that `Down()` is never left broken.

### 5. Order matters in `Down()`

`Down()` must reverse `Up()` in the opposite order. If `Up()` is:

```
pg_trgm → index_a → index_b
```

then `Down()` must be:

```
index_b → index_a → pg_trgm
```
