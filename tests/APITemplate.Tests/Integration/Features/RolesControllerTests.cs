...
public sealed class RolesControllerTests : IClassFixture<CustomWebApplicationFactory>
{
    ...
    public async Task CreateRole_Success()
    {
        ...
        IntegrationAuthHelper.Authenticate(_client, tenantId: tenantId, role: "TenantAdmin", permissions: new[] { "Roles.Create", "Roles.Read" });
    ...
    {
        var tenantId = Guid.NewGuid();
        IntegrationAuthHelper.Authenticate(_client, tenantId: tenantId, role: "TenantAdmin", permissions: new[] { "Roles.Create", "Roles.Update", "Roles.Read" });
    ...
    {
        var tenantId = Guid.NewGuid();
        IntegrationAuthHelper.Authenticate(_client, tenantId: tenantId, role: "TenantAdmin", permissions: new[] { "Roles.Create", "Roles.Delete", "Roles.Read" });
    ...
    {
        var (tenant, user) = await IntegrationAuthHelper.SeedTenantUserAsync(_factory.Services, "assign_role_test", "assign_role_test@test.com");
        
        IntegrationAuthHelper.Authenticate(_client, tenantId: tenant.Id, role: "TenantAdmin", permissions: new[] { "Roles.Create", "Users.Update", "Roles.Read" });
