...
* **Optimistic Concurrency:** PostgreSQL native `xmin` system column is used as a concurrency token — no extra `RowVersion` column is needed. The shared helper `ConfigurePostgresXminConcurrency()` (in `SharedKernel.Infrastructure.Configurations`) registers `xmin` as a shadow property on any entity builder. It is automatically applied to **all** entities that go through `ConfigureTenantAuditable<T>()`, and must be called manually for entities that opt out of that extension (e.g. `FailedEmail`, `CustomRole`, `FileUploadSaga`). `DbUpdateConcurrencyException` is caught by `ApiExceptionHandler` and mapped to HTTP 409. For full end-to-end protection, handlers should expose the current `xmin` value as an `ETag` response header and validate `If-Match` on mutating requests.
