...
| Key                                | Example Value                                   | Description                                                                                                                                                                                         |
|------------------------------------|-------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `Bff:CookieName`                   | `.APITemplate.Auth`                             | Name of the `httpOnly` session cookie issued after a successful BFF login. The cookie contains only a session key — the actual auth ticket is stored in PostgreSQL (cached in Redis).               |
...
| `Bff:RefreshLockTimeoutMilliseconds` | `5000`                                        | Distributed refresh lock TTL in milliseconds. Guards against leader crashes leaving the lock held. |
| `Bff:RefreshResultTtlMilliseconds` | `5000`                                         | How long the refresh coordinator result stays in Redis for late followers to read. |
