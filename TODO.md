...
- [x] **Options classes split between SharedKernel and modules** — module-owned options now live in their modules
  (`Identity`, `Notifications`, `FileStorage`, `BackgroundJobs`). `BackgroundJobsOptions` were moved out of
  `SharedKernel`; Notifications retry flow is decoupled from direct `IOptions<BackgroundJobsOptions>` usage via
  background-jobs-provided runtime settings.
